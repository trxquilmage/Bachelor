...
namespace UnityEngine.UI
{
    ...
    ///
    [ExecuteAlways]
    public abstract class HorizontalOrVerticalLayoutGroupChildren : LayoutGroup
    {
        ...
        /// <param name="isVertical">Is this group a vertical group?</param>
        public virtual void SetChildrenAlongAxis(int axis, bool isVertical)
        {
        ...
        public virtual void GetChildSizes(RectTransform child, int axis, bool controlSize, bool childForceExpand,
            out float min, out float preferred, out float flexible)
        {
            if (!controlSize)
            {
                float startPoint = Mathf.Infinity, endPoint = -Mathf.Infinity;
                foreach (RectTransform rT in child.gameObject.GetComponentsInChildren<RectTransform>())
                {
                    
                    if (startPoint > rT.localPosition[axis])
                        startPoint = rT.localPosition[axis];
                    if (endPoint < rT.localPosition[axis])
                        endPoint = rT.localPosition[axis];
                }
